...
u.save()
    ...
    var q4 = new Quote({author: 'Magnus Carlsen', content: 'Some people think that if their opponent plays a beautiful game, it’s OK to lose. You have to be merciless.'});
    ...
    Promise.all([p1.save(), p2.save(), p3.save()])
        .then(() => {
            console.log('Puzzles saved successfully');
        })
        .catch(err => {
            console.error('Error saving puzzles:', err);
        });
    

User.findOne({ email: 'foo@bar.org' })
    .then(user => {
        var fooId = user.id;
        var g1 = new Game({
            user: fooId,
